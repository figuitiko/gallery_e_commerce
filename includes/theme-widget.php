...
function meris_widgets_init() {
	...
	register_widget('my_post_tabs');
	register_widget('my_expos_tabs');
	register_widget('my_politic');
...
/*
 * widget pa sacar 3 eltos noticias
 * */
 ...
 *  latest posts tabs
 */
class my_post_tabs extends WP_Widget {
	function my_post_tabs () {
		$widget_ops = array( 'classname' => 'mypost ', 'description' => __( 'Display popular posts and latest posts tabs.', 'meris' ) );
		$control_ops = array( 'width' => 250, 'height' =>250 );
		parent::WP_Widget( false, $name = __( 'My: Posts Tabs', 'meris' ), $widget_ops, $control_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
		echo '<div class="mypostags " >
									<div >
										<ul>
  											<li style="color:#ffffff "><p style="font-size: large"><strong>'.__('Noticias', 'meris').'</strong></p></li>
	...
	function tabs_latest_posts( $instance )
	{
		...
		while ($the_query->have_posts()) : $the_query->the_post();
			?>
				...
				<div class="col-xs-12"><div id="phcolor"><a  title="<?php the_title(); ?>" href="<?php the_permalink() ?>"><em>"<?php the_title(); ?>"</em></a><p><?php echo  get_the_date("M d.Y"). "&nbsp;-&nbsp;". get_the_excerpt()?><a href="<?php the_permalink();?>"><div class="entry-more-ini"><?php _e("Leer Más","meris");?> <i class="fa fa-angle-double-right"></i></div></a></p></div></div>
 ...
 *  latest expos tabs
 */
class my_expos_tabs extends WP_Widget {
	function my_expos_tabs () {
		$widget_ops = array( 'classname' => 'myexpos', 'description' => __( 'Display  latest expos tabs.', 'meris' ) );
		$control_ops = array( 'width' => 250, 'height' =>250 );
		parent::WP_Widget( false, $name = __( 'My: Expos Tabs', 'meris' ), $widget_ops, $control_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		echo '<div class="mypostags" >
									<div >
										<ul>
  											<li style="color:#ffffff "><p style="font-size: large"><strong>'.__('Expos', 'meris').'</strong></p></li>
	...
	{
		extract( $instance );
		$the_query = new WP_Query('showposts='. absint($recent_num) .'&orderby=post_date&order=desc&post_type=expos');
 ...
 *  politicas
 */
class my_politic extends WP_Widget {
	function my_politic () {
		$widget_ops = array( 'classname' => 'my_politic', 'description' => __( 'Muestra  politicas.', 'meris' ) );
		$control_ops = array( 'width' => 250, 'height' =>250 );
		parent::WP_Widget( false, $name = __( 'My:Politicas', 'meris' ), $widget_ops, $control_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		echo '<div class="mypostags" >
									<div >
										<ul>
  											<li style="color:#ffffff "><p style="font-size: large"><strong>'.__('Políticas', 'meris').'</strong></p></li>
	...
	{
		extract( $instance );
		$the_query = new WP_Query('showposts=1&p=100&post_type=page');
...
class last_post extends WP_Widget {
	function last_post () {
		$widget_ops = array( 'classname' => 'last_post', 'description' => __( 'Ultimo post.', 'meris' ) );
		$control_ops = array( 'width' => 250, 'height' =>250 );
		parent::WP_Widget( false, $name = __( 'Last:Post', 'meris' ), $widget_ops, $control_ops);
	}
	...
	/*Method to load latest posts*/
	function my_last_post( $instance )
	{
		extract( $instance );
		...
		while ($the_query->have_posts()) : $the_query->the_post();
			...
			<div class="col-xs-4"><div class="col-xs-12"><div class="hovereffect">
						<img class=" img-responsive img-left" src="<?php echo get_the_post_thumbnail_url() ?>" alt="">
						<div class="overlay">
							<h2><?php the_title()?></h2>
							<a class="info" href="<?php  the_permalink()?>">Leer más</a>
						</div>
					</div></div></div>
			<div class="col-xs-8"><div class="col-xs-12"><h1><?php the_title()?></h1><?php the_excerpt() ?><div class="entry-footer">
